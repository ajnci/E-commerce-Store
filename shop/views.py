...
from .download import download_products_pdf, download_products_csv
...
def products_pdf(request):
    if request.user.is_staff:
        ...
        if products.count() > 0:
            response = download_products_pdf(products)
            return response
        else:
            return HttpResponse("There is no product in our shop! Please add products to view this page.")
    else:
        return HttpResponse("Only staff and admin can view this page!")
...
def products_csv(request):
    if request.user.is_staff:
        products = Product.objects.all().order_by('-id')
        if products.count() > 0:
            response = download_products_csv(products)
            return response
        else:
            return HttpResponse("There is no product in our shop! Please add products to download csv file.")
    else:
        return HttpResponse("Only staff and admin can download csv file! You are not allowed here.")