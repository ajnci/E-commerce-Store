...
from .download import download_order_info
...
@login_required(login_url='accounts:login')
def order_info_pdf(request):
    order = Order.objects.filter(user__email=request.user.email, is_ordered=False).order_by('-created_at').first()
    response = download_order_info(order)
    return response
